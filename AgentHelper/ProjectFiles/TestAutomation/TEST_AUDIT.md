...
- Refresh note: Wave 5 revalidated the tooling commands, both pytest slices, and the repo-managed precommit gate before refreshing this audit
...
### 2. Remaining Direct-Coverage Gaps Are Now Concentrated In Explicitly Deferred Legacy Surfaces
...
- Impact:
  ...
  - these areas are now an explicit deferral rather than an accidental omission in the current wave plan
...
- Evidence:
  ...
  - `.\.venv\Scripts\python.exe -m pytest -q -m "docker"` passed with `10 passed, 38 deselected`
...
- The repo-owned discovery, coverage-status, and gap-summary commands all pass from the repository root.
- `tests/test_testing_scripts.py` passed, confirming the Wave 1 tooling helpers remain directly covered.
- The full test landscape still splits cleanly into:
  - a 38-test always-runnable non-Docker subset
  - a 10-test Docker-backed PostgreSQL integration slice
- Coverage remains explicitly unavailable because `pytest-cov` is not installed; this is now a documented deferral rather than an unresolved workflow failure.
- The remaining uncovered files from the gap summary are preserved as explicit follow-up scope instead of being silently expanded into this closeout wave.
...
- legacy training and adjacent experiment surfaces in `src/evaluation.py`, `src/train_flax.py`, `src/mock_trading.py`, and `src/mock_trading_utils.py`
...
1. If you want measured coverage instead of the current advisory-unavailable status, use `$helper-test-preferences` to decide whether `pytest-cov` or another repo-owned coverage path should become supported policy.
2. If testing work continues, use `$helper-test-plan` to create a follow-on wave focused on `src/evaluation.py`, `src/mock_trading.py`, `src/mock_trading_utils.py`, `src/train_flax.py`, `src/utils.py`, `configs/fine_tuning.py`, and `scripts/precommit-checks.mjs`.
3. Keep browser/E2E tooling out of scope unless the repository shape changes beyond the current CLI-first workflows.
