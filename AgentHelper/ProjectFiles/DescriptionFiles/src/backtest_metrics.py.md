...
This module is the Phase 2 source of truth for per-step backtest metric semantics. It keeps the normalized deviation formula, overshoot-versus-undershoot classification, signed percent deviation, and side-of-last-input direction correctness in one pure helper layer that later runtime and PostgreSQL code can reuse without reinterpreting the rules.
